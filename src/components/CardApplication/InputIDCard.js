...
const InputWrap = styled.div`
  ...
  & > label {
    ...
    background-image: linear-gradient(#ddd, #ddd),
      linear-gradient(92deg, #000000 7.01%, #9b988f 72.28%);
...
function InputIDCard({ setProcess, setCardApply, cardApply }) {
  ...
  const gotoNext = () => {
    if (
      (name.length > 0 ? 1 : 0) * personalNumber.length * issueDate.length ===
      112
    ) {
      setCardApply({
        ...cardApply,
  ...
  const handleChange = (event) => {
    if (event.target.files.length > 0) {
      ...
      Tesseract.recognize(tempImagePath, "kor", { logger: (m) => {} })
        .catch((err) => {
          console.error(err);
        })
        .then(({ data: { text } }) => {
  ...
  const changeHandler = (event) => {
    switch (event.target.name) {
      ...
      case "personalNum1":
        ...
        setPersonalNumber(
          event.target.value + "-" + personalNumber.split("-")[1]
        );
        break;
      case "personalNum2":
        ...
        setPersonalNumber(
          personalNumber.split("-")[0] + "-" + event.target.value
        );
        break;
      case "issueDate":
  ...
  return (
    <Background>
      ...
      <MainWrap>
        <div>
          <MainTitle>신분증을 확인하고 진행해주세요</MainTitle>
          <InputWrap image={imagePath}>
            <img
              src={imagePath === "" ? grayBack : imagePath}
            ...
            <div>
              ...
              <InfoInput>
                ...
                <InputNum2
                  ...
                  disabled={personalNumber === ""}
                  value={
                    personalNumber === "" ? "" : personalNumber.split("-")[1]
                  }
                  onChange={changeHandler}
                  name="personalNum2"
                ></InputNum2>
